...
var loadInlayWithStoredFields = function () {
    ...
    let upload_type = ['.bmp', '.doc', '.docx', '.gif', '.jpeg', '.jpg', '.pdf', '.png', '.mp4', '.avi', '.mov', '.mkv', '.JPG', '.JPEG', '.BMP', '.DOC', '.DOCX', '.GIF', '.PDF', '.PNG'];
    console.log(JSON.stringify(upload_type));

    // let upload_type = '.bmp,.doc,.docx,.gif,.jpeg,.jpg,.pdf,.png,.JPG,.JPEG,.BMP,.DOC,.DOCX,.GIF,.PDF,.PNG';
    // let upload_type = '.bmp','.doc','.docx','.gif','.jpeg','.jpg','.pdf','.png','.JPG','.JPEG','.BMP','.DOC','.DOCX','.GIF','.PDF','.PNG';
...
function getSubject() {
    ...
    if (reason === '7.1 surround sound') {
        ...
        switch (getMouseData('case_reason')) {
            case 1:
                subject = 'lRos9 ~1~';
                break;
            case 2:
...
function createMouseUtterance() {
    ...
    mouseData.push(getMouseData('case_model'));
