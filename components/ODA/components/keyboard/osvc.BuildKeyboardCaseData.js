...
function metadata(){
    return {
        name: 'osvc.BuildKeyboardCaseData',
        properties: {
...
async function invoke(context){
    ...
    context.logger().info("------ BUILD KEYBOARD CASE DATA START ------");

    try{

        context.logger().info("------ osvc.BuildKeyboardCaseData: extract parameters ------");
        const {
        ...
        context.logger().info("------ osvc.BuildKeyboardCaseData: build notes ------");
        ...
        context.logger().info("------ osvc.BuildKeyboardCaseData: build files ------");
        const files = [];
        ...
        context.logger().info("------ osvc.BuildKeyboardCaseData: build case data ------");
        let caseData = {
        ...
        context.logger().info("------ osvc.BuildKeyboardCaseData: success ------");
        context.keepTurn(true);
        context.transition('next');
    }catch(error){
        context.logger().info("------ osvc.BuildKeyboardCaseData: error ------");
        context.keepTurn(true);
        context.transition('error');
    }

    context.logger().info("------ BUILD KEYBOARD CASE DATA END ------");
    return;
}
