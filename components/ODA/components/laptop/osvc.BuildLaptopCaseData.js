...
async function invoke(context){
    ...
    context.logger().info("------ BUILD LAPTOP CASE DATA START ------");

    try{

        context.logger().info("------ osvc.BuildLaptopCaseData: extract parameters ------");
        const {
        ...
        context.logger().info("------ osvc.BuildLaptopCaseData: build notes ------");
        ...
        context.logger().info("------ osvc.BuildLaptopCaseData: build files ------");
        const files = [];
        ...
        context.logger().info("------ osvc.BuildLaptopCaseData: success ------");
        context.keepTurn(true);
        context.transition('next');
    }catch(error){
        context.logger().info("------ osvc.BuildLaptopCaseData: error ------");
        context.keepTurn(true);
        context.transition('error');
    }

    context.logger().info("------ BUILD LAPTOP CASE DATA END ------");
    return;
}
