...
const symptomsIndexMap = require("../../json/laptop/symptoms-index-map.json");
...
function metadata(){
  return {
    name: 'osvc.EvaluateLaptopSymptoms',
    properties: {
        // GETTERS
...
async function invoke(context){
    context.logger().info("------ EVALUATE LAPTOP SYMPTOMS START ------");
    try{
        const {
            issueKeyword, input,
            symptomsVar
        ...
        context.logger().info(`osvc.EvaluateLaptopSymptoms: issue keyword ${issueKeyword}`);
        context.logger().info(`osvc.EvaluateLaptopSymptoms: input ${input}`);

        let sanitizedInput= input.replace(/[,;\-\.]/g,' ').trim();
        let selections = sanitizedInput.split(' ');

        context.logger().info(`osvc.EvaluateLaptopSymptoms: selections ${selections}`);
    ...
    context.logger().info("------ EVALUATE LAPTOP SYMPTOMS END   ------");
    return;
}
