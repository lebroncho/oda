...
async function invoke(context){
    ...
    context.logger().info("------ BUILD MOUSE CASE DATA START ------");

    try{

        context.logger().info("------ osvc.BuildMouseCaseData: extract parameters ------");
        const {
        ...
        context.logger().info("------ osvc.BuildMouseCaseData: build notes ------");
        ...
        context.logger().info("------ osvc.BuildMouseCaseData: build files ------");
        const files = [];
        ...
        context.logger().info("------ osvc.BuildMouseCaseData: build case data ------");
        let caseData = {
        ...
        context.logger().info("------ osvc.BuildMouseCaseData: success ------");
        context.keepTurn(true);
        context.transition('next');
    }catch(error){
        context.logger().info("------ osvc.BuildMouseCaseData: error ------");
        context.keepTurn(true);
        context.transition('error');
    }

    context.logger().info("------ BUILD MOUSE CASE DATA END ------");
    return;
}
