...
function metadata(){
    return {
        name: 'osvc.BuildControllerCaseData',
        properties: {
...
async function invoke(context){
    ...
    context.logger().info("------ BUILD CONTROLLER CASE DATA START ------");

    try{

        context.logger().info("------ osvc.BuildControllerCaseData: extract parameters ------");
        const {
        ...
        context.logger().info("------ osvc.BuildControllerCaseData: build notes ------");
        ...
        context.logger().info("------ osvc.BuildControllerCaseData: build files ------");
        const files = [];
        ...
        context.logger().info("------ osvc.BuildControllerCaseData: build case data ------");
        let caseData = {
        ...
        context.logger().info("------ osvc.BuildControllerCaseData: success ------");
        context.keepTurn(true);
        context.transition('next');
    }catch(error){
        context.logger().info("------ osvc.BuildControllerCaseData: error ------");
        context.keepTurn(true);
        context.transition('error');
    }

    context.logger().info("------ BUILD CONTROLLER CASE DATA END ------");
    return;
}
