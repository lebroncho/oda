...
class SetIncidentOnChat {
  ...
  metadata() {
    return {
      name: "osvc.SetIncidentOnChat",
      properties: {
        incidentId: { type: "int", required: true },
        chatSessionId: { type: "string", required: true },
      },
      supportedActions: ["incidentSetOnChat", "fail"],
    };
  }

  invoke(conversation, done) {
    conversation.logger().info("------ SetIncidentOnChat Invoked ------");
    ...
    const { incidentId, chatSessionId } = conversation.properties();
    ...
    (async () => {
      try {
        ...
        const setIncident = await this.chatApiClient.request(
          chatServerDomain,
        ...
        conversation.logger().info(setIncident);
        conversation.logger().info("------");
        ...
        conversation.keepTurn(true).transition("incidentSetOnChat");
        done();
      } catch (error) {
...
module.exports = SetIncidentOnChat;
